    ...
    <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #ddd; border-radius: 5px; padding: 20px; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1);">
        ...
        <div style="background-color: #f4f4f4; padding: 10px; border-radius: 5px; margin-top: 20px;">
            ...
            <ul style="list-style-type: none; padding: 0;">
                <li style="margin-bottom: 10px;"><strong>User:</strong> ${
                  subscription.user.email
                }</li>
                <li style="margin-bottom: 10px;"><strong>Plan:</strong> ${
                  subscription.plan.name
                }</li>
                <li style="margin-bottom: 10px;"><strong>Start Date:</strong> ${moment(
                  subscription.startDate
                ).format("YYYY-MM-DD")}</li>
                <li style="margin-bottom: 10px;"></strong>Expiry Date:</strong> ${moment(
...
`;
    ...
    const mailOptions = {
      ...
      subject: `Orignify: Subscription Expired - ${moment(
        subscription.expiryDate
      ).format("YYYY-MM-DD")}`,
      html: htmlBody,
    };
    ...
    console.error(
      "Error updating subscription status and sending email:",
      error
    );
  }
}
...
const task = cron.schedule("0 0 * * *", async () => {
  try {
    ...
    await Promise.all(
      subscriptionsToExpire.map((subs) => updateSubcriptionAndSendMail(subs))
    );
